/*
ZET local dev server — standard scaffold used by `zet create`.
Serves the built widget from /app over https on 127.0.0.1.

key.pem / cert.pem are not included. Generate once with:
  openssl req -nodes -new -x509 -keyout key.pem -out cert.pem
and place both in the project root before running `npm start`.
*/
...
portPromise.then((port) => {
  ...
  expressApp.get('/', function (req, res) {
    res.redirect('/app/react/index.html');
  });
  ...
  https.createServer(options, expressApp).listen(port, function () {
    ...
    console.log(chalk.bold.cyan("Enable https://127.0.0.1:" + port + " in a new tab and click Advanced -> Proceed, then use https://127.0.0.1:" + port + "/app/react/index.html as the widget URL in Zoho Creator."));
